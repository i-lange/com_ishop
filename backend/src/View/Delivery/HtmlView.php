...
namespace Ilange\Component\Ishop\Administrator\View\Delivery;
...
/**
 * Класс представления для способа доставки
 * @since 1.0.0
 */
class HtmlView extends FormView
{
    ...
    public function __construct(array $config)
    {
        ...
        $config['help_link']      = 'Delivery:_Edit';
        $config['toolbar_icon']   = 'pencil-alt delivery-add';
    ...
    protected function addToolbar()
    {
        ...
        $this->toolbarTitle = Text::_('COM_ISHOP_' . ($checkedOut ? 'VIEW' : ($isNew ? 'ADD' : 'EDIT')) . '_DELIVERY');
    ...
    protected function addModalToolbar()
    {
        ...
        ToolbarHelper::title(
            Text::_('COM_ISHOP_' . ($checkedOut ? 'VIEW' : ($isNew ? 'ADD' : 'EDIT') . '_DELIVERY')),
            'pencil-alt delivery-add'
        );
        ...
        if ($canCreate || $canEdit) {
            $toolbar->apply('delivery.apply');
            $toolbar->save('delivery.save');
        }

        $toolbar->cancel('delivery.cancel');
    }
}
