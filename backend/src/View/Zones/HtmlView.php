...
namespace Ilange\Component\Ishop\Administrator\View\Zones;

defined('_JEXEC') or die;

use Ilange\Component\Ishop\Administrator\Extension\IshopComponent;
use Ilange\Component\Ishop\Administrator\Model\ZonesModel;
...
class HtmlView extends BaseHtmlView
{
    ...
    protected function addToolbar()
    {
        ...
        // Заголовок страницы
        ToolbarHelper::title(Text::_('COM_ISHOP_ZONES'), 'fa fa-cubes');
        ...
        if (!$this->isEmptyState) {
            ...
            if ($canDo->get('core.edit.state')) {
                ...
                $childBar->archive('zones.archive')->listCheck(true);
                // Кнопка разблокировать
                $childBar->checkin('zones.checkin');
            ...
            if ($this->state->get('filter.published') == IshopComponent::CONDITION_TRASHED &&
                ...
                $toolbar->delete('zones.delete')
                    ->message('JGLOBAL_CONFIRM_DELETE')
                    ->listCheck(true);
            }
        }

        // Кнопка обновления доступных дат доставки
        $toolbar
            ->standardButton(
                'update',
                'COM_ISHOP_ZONES_UPDATE',
                'zones.update')
            ->icon('icon-refresh');
