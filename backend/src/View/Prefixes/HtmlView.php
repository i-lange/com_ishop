...
namespace Ilange\Component\Ishop\Administrator\View\Prefixes;

defined('_JEXEC') or die;

use Ilange\Component\Ishop\Administrator\Extension\IshopComponent;
use Ilange\Component\Ishop\Administrator\Model\PrefixesModel;
...
class HtmlView extends BaseHtmlView
{
    ...
    {
        $canDo	 = ContentHelper::getActions('com_ishop', 'prefix');
        $user    = $this->getCurrentUser();
        $toolbar = $this->getDocument()->getToolbar();

        // Заголовок страницы
        ToolbarHelper::title(Text::_('COM_ISHOP_PREFIXES'), 'fa fa-cubes');

        // Кнопка создания
        if ($canDo->get('core.create')) {
            $toolbar->addNew('prefix.add');
        }
        ...
        if (!$this->isEmptyState) {
            ...
            if ($canDo->get('core.edit.state')) {
                // Кнопка опубликовать
                $childBar->publish('prefixes.publish')->listCheck(true);
                // Кнопка снять с публикации
                $childBar->unpublish('prefixes.unpublish')->listCheck(true);
                // Кнопка переместить в архив
                $childBar->archive('prefixes.archive')->listCheck(true);
                // Кнопка разблокировать
                $childBar->checkin('prefixes.checkin');

                // Кнопка перемещения в корзину
                if ($this->state->get('filter.published') != IshopComponent::CONDITION_TRASHED) {
                    $childBar->trash('prefixes.trash')->listCheck(true);
                }
            }
            ...
            if ($this->state->get('filter.published') == IshopComponent::CONDITION_TRASHED &&
                ...
                $toolbar->delete('prefixes.delete')
