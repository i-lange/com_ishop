...
class ZoneModel extends AdminModel
{
     ...
     * @since 1.0.0
     */
    public $typeAlias = 'com_ishop.zone';
    ...
    public function getForm($data = [], $loadData = true)
    {
        ...
        $id         = (int)$this->getState('zone.id', $IdFromInput);
        $record->id = $id;
    ...
    protected function loadFormData()
    {
        ...
        $data = Factory::getApplication()->getUserState('com_ishop.edit.zone.data', []);
        ...
        if (empty($data)) {
            ...
            if ($this->getState('zone.id') == 0) {
                $filters = (array)$app->getUserState('com_ishop.zone.filter');
        ...
        $this->preprocessData('com_ishop.zone', $data);
    ...
    public function getItem($pk = null)
    {
        ...
        // Преобразуем параметры в массив
        $registry      = new Registry($item->current);
        $item->current = $registry->toArray();
     ...
     * @since 1.0.0
     */
    public function getTable($name = 'Zone', $prefix = '', $options = [])
    {
