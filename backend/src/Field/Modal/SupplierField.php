...
/**
 * Класс модального поля Поставщик
 * @since 1.0.0
 */
class SupplierField extends ModalSelectField
{
    /**
     * Тип поля формы
     * @var string
    ...
    public function setup(\SimpleXMLElement $element, $value, $group = null)
    {
        ...
        if ($language) {
            ...
            $modalTitle = Text::_('COM_ISHOP_SELECT_SUPPLIER') . ' &#8212; ' . $this->getTitle();

            $this->dataAttributes['data-language'] = $language;
        } else {
            $modalTitle = Text::_('COM_ISHOP_SELECT_SUPPLIER');
        }

        $urlSelect = $linkItems;
        $urlEdit   = clone $linkItem;
        $urlEdit->setVar('task', 'supplier.edit');
        $urlNew    = clone $linkItem;
        $urlNew->setVar('task', 'supplier.add');
        ...
        $this->modalTitles['new']    = Text::_('COM_ISHOP_ADD_SUPPLIER');
        $this->modalTitles['edit']   = Text::_('COM_ISHOP_EDIT_SUPPLIER');

        $this->hint = $this->hint ?: Text::_('COM_ISHOP_SELECT_SUPPLIER');
