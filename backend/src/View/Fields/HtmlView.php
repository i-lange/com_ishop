...
namespace Ilange\Component\Ishop\Administrator\View\Fields;

defined('_JEXEC') or die;

use Ilange\Component\Ishop\Administrator\Extension\IshopComponent;
use Ilange\Component\Ishop\Administrator\Model\FieldsModel;
...
class HtmlView extends BaseHtmlView
{
    ...
    {
        $canDo	 = ContentHelper::getActions('com_ishop', 'field', $this->state->get('filter.group_id'));
        $user    = $this->getCurrentUser();
        $toolbar = $this->getDocument()->getToolbar();

        // Заголовок страницы
        ToolbarHelper::title(Text::_('COM_ISHOP_FIELDS'), 'fa fa-cubes');
        ...
        if (!$this->isEmptyState) {
            ...
            if ($canDo->get('core.edit.state')) {
                ...
                $childBar->checkin('fields.checkin');
        ...
        // Кнопка ссылка на представление списка групп характеристик
        $toolbar
            ->linkButton('groups', 'COM_ISHOP_GROUPS')
            ->url('index.php?option=com_ishop&view=groups')
            ->icon('icon-eye');

        // Кнопка обновления дополнительных таблиц для фильтрации товаров по характеристикам
        if (!$this->isEmptyState) {
            $toolbar->standardButton('update_filter', 'COM_ISHOP_FIELDS_UPDATE_FILTER', 'fields.update_filter')
                ->icon('icon-refresh');
        }
