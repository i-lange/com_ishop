...
class ProductField extends ModalSelectField
{
    ...
    public function setup(\SimpleXMLElement $element, $value, $group = null)
    {
        ...
        if ($language) {
            ...
            $modalTitle = Text::_('COM_ISHOP_SELECT_PRODUCT') . ' &#8212; ' . $this->getTitle();

            $this->dataAttributes['data-language'] = $language;
        } else {
            $modalTitle = Text::_('COM_ISHOP_SELECT_PRODUCT');
        }

        $urlSelect = $linkItems;
        $urlEdit   = clone $linkItem;
        $urlEdit->setVar('task', 'product.edit');
        $urlNew    = clone $linkItem;
        $urlNew->setVar('task', 'product.add');
        ...
        $this->modalTitles['new']    = Text::_('COM_ISHOP_ADD_PRODUCT');
        $this->modalTitles['edit']   = Text::_('COM_ISHOP_EDIT_PRODUCT');

        $this->hint = $this->hint ?: Text::_('COM_ISHOP_SELECT_PRODUCT');
