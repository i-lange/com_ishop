...
namespace Ilange\Component\Ishop\Administrator\View\Parts;

defined('_JEXEC') or die;

use Ilange\Component\Ishop\Administrator\Extension\IshopComponent;
use Ilange\Component\Ishop\Administrator\Model\PartsModel;
...
class HtmlView extends BaseHtmlView
{
    ...
    protected function addToolbar()
    {
        ...
        // Заголовок страницы
        ToolbarHelper::title(Text::_('COM_ISHOP_PARTS'), 'fa fa-cubes');

        // Кнопка создания
        if ($canDo->get('core.create')) {
            $toolbar->addNew('part.add');
        }
        ...
        if (!$this->isEmptyState) {
            ...
            if ($canDo->get('core.edit.state')) {
                // Кнопка опубликовать
                $childBar->publish('parts.publish')->listCheck(true);
                // Кнопка снять с публикации
                $childBar->unpublish('parts.unpublish')->listCheck(true);
                // Кнопка переместить в архив
                $childBar->archive('parts.archive')->listCheck(true);
                // Кнопка разблокировать
                $childBar->checkin('parts.checkin');

                // Кнопка перемещения в корзину
                if ($this->state->get('filter.published') != IshopComponent::CONDITION_TRASHED) {
                    $childBar->trash('parts.trash')->listCheck(true);
                }
            }
