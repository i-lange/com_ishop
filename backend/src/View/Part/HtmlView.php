...
namespace Ilange\Component\Ishop\Administrator\View\Part;
...
class HtmlView extends FormView
{
    ...
    public function __construct(array $config)
    {
        ...
        $config['help_link']      = 'Part:_Edit';
        $config['toolbar_icon']   = 'pencil-alt part-add';
    ...
    protected function initializeView()
    {
        ...
        $this->canDo = ContentHelper::getActions('com_ishop', 'part', $this->item->id);
    ...
    protected function addToolbar()
    {
        ...
        $this->toolbarTitle = Text::_('COM_ISHOP_' . ($checkedOut ? 'VIEW' : ($isNew ? 'ADD' : 'EDIT')) . '_PART');
    ...
    protected function addModalToolbar()
    {
        ...
        ToolbarHelper::title(
            Text::_('COM_ISHOP_' . ($checkedOut ? 'VIEW' : ($isNew ? 'ADD' : 'EDIT') . '_PART')),
            'pencil-alt part-add'
        );
        ...
        if ($canCreate || $canEdit) {
            $toolbar->apply('part.apply');
            $toolbar->save('part.save');
        }

        $toolbar->cancel('part.cancel');
    }
}
