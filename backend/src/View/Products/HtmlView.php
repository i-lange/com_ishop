...
namespace Ilange\Component\Ishop\Administrator\View\Products;

defined('_JEXEC') or die;

use Ilange\Component\Ishop\Administrator\Extension\IshopComponent;
use Ilange\Component\Ishop\Administrator\Model\ProductsModel;
...
class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        if ($this->getLayout() !== 'modal') {
            ...
            // Нам не нужно выполнять фильтрацию по языку, если мультиязычность отключена
    ...
    {
        $canDo	 = ContentHelper::getActions('com_ishop', 'category', $this->state->get('filter.category_id'));
        $user    = $this->getCurrentUser();
        $toolbar = $this->getDocument()->getToolbar();

        // Заголовок страницы
        ToolbarHelper::title(Text::_('COM_ISHOP_PRODUCTS'), 'fa fa-cubes');

        // Кнопка создания нового товара
        if ($canDo->get('core.create') || !empty($user->getAuthorisedCategories('com_ishop', 'core.create'))) {
        ...
        if (!$this->isEmptyState) {
            ...
            if ($this->state->get('filter.published') == IshopComponent::CONDITION_TRASHED && $canDo->get('core.delete')) {
        ...
        // Кнопка ссылка на представление списка префиксов для товаров
        $toolbar
            ->linkButton('prefixes', 'COM_ISHOP_PREFIXES')
            ->url('index.php?option=com_ishop&view=prefixes')
            ->icon('icon-eye');

        // Кнопка ссылка на настройки компонента iShop
        if ($user->authorise('core.admin', 'com_ishop') || $user->authorise('core.options', 'com_ishop')) {
