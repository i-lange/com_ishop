...
namespace Ilange\Component\Ishop\Administrator\View\Deliveries;

defined('_JEXEC') or die;

use Ilange\Component\Ishop\Administrator\Extension\IshopComponent;
use Ilange\Component\Ishop\Administrator\Model\DeliveriesModel;
...
class HtmlView extends BaseHtmlView
{
    ...
    protected function addToolbar()
    {
        ...
        // Заголовок страницы
        ToolbarHelper::title(Text::_('COM_ISHOP_DELIVERIES'), 'fa fa-cubes');

        // Кнопка создания
        if ($canDo->get('core.create')) {
            $toolbar->addNew('delivery.add');
        }
        ...
        if (!$this->isEmptyState) {
            ...
            if ($canDo->get('core.edit.state')) {
                // Кнопка опубликовать
                $childBar->publish('deliveries.publish')->listCheck(true);
                // Кнопка снять с публикации
                $childBar->unpublish('deliveries.unpublish')->listCheck(true);
                // Кнопка переместить в архив
                $childBar->archive('deliveries.archive')->listCheck(true);
                // Кнопка разблокировать
                $childBar->checkin('deliveries.checkin');

                // Кнопка перемещения в корзину
                if ($this->state->get('filter.published') != IshopComponent::CONDITION_TRASHED) {
                    $childBar->trash('deliveries.trash')->listCheck(true);
                }
            }
            ...
            if ($this->state->get('filter.published') == IshopComponent::CONDITION_TRASHED &&
                ...
                $toolbar->delete('deliveries.delete')
