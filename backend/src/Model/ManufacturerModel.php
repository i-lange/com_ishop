...
class ManufacturerModel extends AdminModel
{
     ...
     * @since 1.0.0
     */
    public $typeAlias = 'com_ishop.manufacturer';
     ...
     * @since  1.0.0
     */
    protected $associationsContext = 'com_ishop.manufacturer';
    ...
    protected function canDelete($record)
    {
        ...
        return $this->getCurrentUser()->authorise('core.delete', 'com_ishop.manufacturer.' . (int)$record->id);
    }
    ...
    protected function canEditState($record)
    {
        ...
        // Проверка для существующей записи
        if (!empty($record->id)) {
            return $user->authorise('core.edit.state', 'com_ishop.manufacturer.' . (int)$record->id);
        }
    ...
    public function getForm($data = [], $loadData = true)
    {
        ...
        // Получаем ID записи, для фронтенда используем a_id, а для бэкенда - id
        $IdFromInput = $app->isClient('site')
            ? $app->getInput()->getInt('a_id', 0)
            : $app->getInput()->getInt('id', 0);

        // При редактировании производителя
        // получаем идентификатор из состояния manufacturer.id,
        // при сохранении используем данные из input
        $id         = (int)$this->getState('manufacturer.id', $IdFromInput);
        $record->id = $id;
    ...
    protected function loadFormData()
    {
        ...
        $data = Factory::getApplication()->getUserState('com_ishop.edit.manufacturer.data', []);
        ...
        if (empty($data)) {
            ...
            if ($this->getState('manufacturer.id') == 0) {
                $filters = (array)$app->getUserState('com_ishop.manufacturer.filter');
        ...
        $this->preprocessData('com_ishop.manufacturer', $data);
     ...
     * @since 1.0.0
     */
    public function getTable($name = 'Manufacturer', $prefix = '', $options = [])
    {
