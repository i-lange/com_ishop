...
namespace Ilange\Component\Ishop\Administrator\View\Order;
...
/**
 * Класс представления для заказа
 * @since 1.0.0
 */
class HtmlView extends FormView
{
    ...
    public function __construct(array $config)
    {
        ...
        $config['help_link']      = 'Order:_Edit';
        $config['toolbar_icon']   = 'pencil-alt order-add';
    ...
    protected function initializeView()
    {
        ...
        $this->canDo = ContentHelper::getActions('com_ishop', 'order', $this->item->id);
    ...
    protected function addToolbar()
    {
        ...
        $this->toolbarTitle = Text::_('COM_ISHOP_' . ($checkedOut ? 'VIEW' : ($isNew ? 'ADD' : 'EDIT')) . '_ORDER');
    ...
    protected function addModalToolbar()
    {
        ...
        ToolbarHelper::title(
            Text::_('COM_ISHOP_' . ($checkedOut ? 'VIEW' : ($isNew ? 'ADD' : 'EDIT') . '_ORDER')),
            'pencil-alt order-add'
        );
