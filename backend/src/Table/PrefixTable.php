...
class PrefixTable extends Table
{
	...
	{
        $this->typeAlias = 'com_ishop.prefix';
		parent::__construct('#__ishop_prefixes', 'id', $db, $dispatcher);
    ...
    public function check()
    {
        ...
        if (!$this->id) {
            ...
            // Единственное число по умолчанию равно названию
            if (!isset($this->nominative_sin)) {
                $this->nominative_sin = $this->title;
            }

            // Множественное число по умолчанию пустая строка
            if (!isset($this->nominative_plu)) {
                $this->nominative_plu = $this->title;
            }

            // Родительный по умолчанию пустая строка
            if (!isset($this->genitive_sin)) {
                $this->genitive_sin = $this->title;
            }

            // Родительный во множественном числе по умолчанию пустая строка
            if (!isset($this->genitive_plu)) {
                $this->genitive_plu = $this->title;
            }

            // Винительный по умолчанию пустая строка
            if (!isset($this->accusative_sin)) {
                $this->accusative_sin = $this->title;
            }

            // Винительный во множественном числе по умолчанию пустая строка
            if (!isset($this->accusative_plu)) {
                $this->accusative_plu = $this->title;
            }
        }
