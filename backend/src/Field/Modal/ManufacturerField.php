...
/**
 * Класс модального поля Производитель
 * @since 1.0.0
 */
class ManufacturerField extends ModalSelectField
{
    /**
     * Тип поля формы
     * @var string
    ...
    public function setup(\SimpleXMLElement $element, $value, $group = null)
    {
        ...
        if ($language) {
            ...
            $modalTitle = Text::_('COM_ISHOP_SELECT_MANUFACTURER') . ' &#8212; ' . $this->getTitle();

            $this->dataAttributes['data-language'] = $language;
        } else {
            $modalTitle = Text::_('COM_ISHOP_SELECT_MANUFACTURER');
        }

        $urlSelect = $linkItems;
        $urlEdit   = clone $linkItem;
        $urlEdit->setVar('task', 'manufacturer.edit');
        $urlNew    = clone $linkItem;
        $urlNew->setVar('task', 'manufacturer.add');
        ...
        $this->modalTitles['new']    = Text::_('COM_ISHOP_NEW_MANUFACTURER');
        $this->modalTitles['edit']   = Text::_('COM_ISHOP_EDIT_MANUFACTURER');

        $this->hint = $this->hint ?: Text::_('COM_ISHOP_SELECT_MANUFACTURER');
