...
class DiscountModel extends AdminModel
{
     ...
     * @since 1.0.0
     */
    public $typeAlias = 'com_ishop.discount';
    ...
    protected function canDelete($record)
    {
        ...
        return $this->getCurrentUser()->authorise('core.delete', 'com_ishop.discount.' . (int)$record->id);
    }
    ...
    protected function canEditState($record)
    {
        ...
        if (!empty($record->id)) {
            return $user->authorise('core.edit.state', 'com_ishop.discount.' . (int)$record->id);
        }
    ...
    {
        // ID товаров
        if (!isset($table->products) || !is_array($table->products)) {
            $table->products = [];
        } else {
            // удалить пустые элементы массива
            $table->products = array_diff($table->products, [0, null]);
        }
        // преобразуем в строку
        $table->products = implode(',', $table->products);

        // ID категорий
        if (!isset($table->cats) || !is_array($table->cats)) {
            $table->cats = [];
        } else {
            // удалить пустые элементы массива
            $table->cats = array_diff($table->cats, [0, null]);
        }
        // преобразуем в строку
        $table->cats = implode(',', $table->cats);

        // ID производителей
        if (!isset($table->manufacturers) || !is_array($table->manufacturers)) {
            $table->manufacturers = [];
        } else {
            // удалить пустые элементы массива
            $table->manufacturers = array_diff($table->manufacturers, [0, null]);
        }
        // преобразуем в строку
        $table->manufacturers = implode(',', $table->manufacturers);

        // ID поставщиков
        if (!isset($table->suppliers) || !is_array($table->suppliers)) {
            $table->suppliers = [];
        } else {
            // удалить пустые элементы массива
            $table->suppliers = array_diff($table->suppliers, [0, null]);
        }
        // преобразуем в строку
        $table->suppliers = implode(',', $table->suppliers);

        // min_price
        if (empty($table->min_price) || !is_numeric($table->min_price)) {
            $table->min_price = 0;
        }

        // min_amount
        if (empty($table->min_amount) || !is_numeric($table->min_amount)) {
            $table->min_amount = 0;
        }
    }
    ...
    public function getForm($data = [], $loadData = true)
    {
        ...
        $id         = (int)$this->getState('discount.id', $IdFromInput);
        $record->id = $id;
    ...
    protected function loadFormData()
    {
        ...
        $data = Factory::getApplication()->getUserState('com_ishop.edit.discount.data', []);
        ...
        if (empty($data)) {
            ...
            if ($this->getState('discount.id') == 0) {
                $filters = (array)$app->getUserState('com_ishop.discount.filter');
        ...
        $this->preprocessData('com_ishop.discount', $data);
    ...
    public function getItem($pk = null)
    {
        ...
        // Преобразуем список производителей
        if (!empty($item->manufacturers)) {
            $item->manufacturers = explode(',', $item->manufacturers);
        } else {
            $item->manufacturers = [];
        }

        // Преобразуем список поставщиков
        if (!empty($item->suppliers)) {
            $item->suppliers = explode(',', $item->suppliers);
        } else {
            $item->suppliers = [];
        }
     ...
     * @since 1.0.0
     */
    public function getTable($name = 'Discount', $prefix = '', $options = [])
    {
