 ...
 * @since 1.0.0
 */
class PrefixModel extends AdminModel
{
    /**
     ...
     * @since 1.0.0
     */
    public $typeAlias = 'com_ishop.prefix';
     ...
     * @since  1.0.0
     */
    protected $associationsContext = 'com_ishop.prefix';
    ...
    protected function canDelete($record)
    {
        ...
        return $this->getCurrentUser()->authorise('core.delete', 'com_ishop.prefix.' . (int)$record->id);
    }
    ...
    protected function canEditState($record)
    {
        ...
        if (!empty($record->id)) {
            return $user->authorise('core.edit.state', 'com_ishop.prefix.' . (int)$record->id);
        }
    ...
    public function getForm($data = [], $loadData = true)
    {
        ...
        // При редактировании
        // получаем идентификатор из состояния prefix.id,
        // при сохранении используем данные из input
        $id         = (int)$this->getState('prefix.id', $IdFromInput);
        $record->id = $id;
    ...
    protected function loadFormData()
    {
        ...
        $data = Factory::getApplication()->getUserState('com_ishop.edit.prefix.data', []);
        ...
        if (empty($data)) {
            ...
            if ($this->getState('prefix.id') == 0) {
                $filters = (array)$app->getUserState('com_ishop.prefix.filter');
        ...
        $this->preprocessData('com_ishop.prefix', $data);
     ...
     * @since 1.0.0
     */
    public function getTable($name = 'Prefix', $prefix = '', $options = [])
    {
