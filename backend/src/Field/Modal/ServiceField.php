...
/**
 * Класс модального поля Сервисный центр
 * @since 1.0.0
 */
class ServiceField extends ModalSelectField
{
    /**
     * Тип поля формы
     * @var string
    ...
    public function setup(\SimpleXMLElement $element, $value, $group = null)
    {
        ...
        if ($language) {
            ...
            $modalTitle = Text::_('COM_ISHOP_SELECT_SERVICE') . ' &#8212; ' . $this->getTitle();

            $this->dataAttributes['data-language'] = $language;
        } else {
            $modalTitle = Text::_('COM_ISHOP_SELECT_SERVICE');
        }

        $urlSelect = $linkItems;
        $urlEdit   = clone $linkItem;
        $urlEdit->setVar('task', 'service.edit');
        $urlNew    = clone $linkItem;
        $urlNew->setVar('task', 'service.add');
        ...
        $this->modalTitles['new']    = Text::_('COM_ISHOP_ADD_SERVICE');
        $this->modalTitles['edit']   = Text::_('COM_ISHOP_EDIT_SERVICE');

        $this->hint = $this->hint ?: Text::_('COM_ISHOP_SELECT_SERVICE');
