...
class WarehousesModel extends ListModel
{
    ...
    protected function populateState($ordering = 'a.id', $direction = 'asc')
    {
        ...
        $point = $this->getUserStateFromRequest($this->context . '.filter.point', 'filter_point', '');
    ...
    protected function getListQuery()
    {
            ...
            $subQuery = $db->getQuery(true)
                ...
                ->where(
                    [
                        ...
                        $db->quoteName('asso1.context') . ' = ' . $db->quote('com_ishop.warehouse'),
                    ]
                );
        ...
        // Фильтрация по признаку пункта выдачи
        ...
        if (is_numeric($point)) {
            ...
            $point = [0, 1];
            $query->whereIn($db->quoteName('a.point'), $point);
        } elseif (is_array($point)) {
            $point = ArrayHelper::toInteger($point);
            $query->whereIn($db->quoteName('a.point'), $point);
        }
    ...
    public function getItems()
    {
        ...
        foreach ($items as $item) {
            $item->typeAlias = 'com_ishop.warehouses';
        }
