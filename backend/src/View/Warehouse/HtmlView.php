...
namespace Ilange\Component\Ishop\Administrator\View\Warehouse;
...
/**
 * Класс представления для склада
 * @since 1.0.0
 */
class HtmlView extends FormView
{
    ...
    public function __construct(array $config)
    {
        ...
        $config['help_link']      = 'Warehouse:_Edit';
        $config['toolbar_icon']   = 'pencil-alt warehouse-add';
    ...
    protected function initializeView()
    {
        ...
        $this->canDo = ContentHelper::getActions('com_ishop', 'warehouse', $this->item->id);
    ...
    protected function addToolbar()
    {
        ...
        $this->toolbarTitle = Text::_('COM_ISHOP_' . ($checkedOut ? 'VIEW' : ($isNew ? 'ADD' : 'EDIT')) . '_WAREHOUSE');
    ...
    protected function addModalToolbar()
    {
        ...
        ToolbarHelper::title(
            Text::_('COM_ISHOP_' . ($checkedOut ? 'VIEW' : ($isNew ? 'ADD' : 'EDIT') . '_WAREHOUSE')),
            'pencil-alt warehouse-add'
        );
        ...
        if ($canCreate || $canEdit) {
            $toolbar->apply('warehouse.apply');
            $toolbar->save('warehouse.save');
        }

        $toolbar->cancel('warehouse.cancel');
    }
}
