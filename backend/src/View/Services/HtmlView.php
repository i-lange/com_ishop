...
namespace Ilange\Component\Ishop\Administrator\View\Services;

defined('_JEXEC') or die;

use Ilange\Component\Ishop\Administrator\Extension\IshopComponent;
use Ilange\Component\Ishop\Administrator\Model\ServicesModel;
...
/**
 * Класс отображения списка сервисных центров
 * @since 1.0.0
 */
class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        $this->root				= Uri::root();
        $this->hits = ComponentHelper::getParams('com_ishop')->get('record_hits', 1);
    ...
    protected function addToolbar()
    {
        ...
        // Заголовок страницы
        ToolbarHelper::title(Text::_('COM_ISHOP_SERVICES'), 'fa fa-cubes');
        ...
        if (!$this->isEmptyState) {
            ...
            if ($canDo->get('core.edit.state')) {
                // Кнопка опубликовать
                $childBar->publish('services.publish')->listCheck(true);
                // Кнопка снять с публикации
                $childBar->unpublish('services.unpublish')->listCheck(true);
                // Кнопка переместить в архив
                $childBar->archive('services.archive')->listCheck(true);
                // Кнопка разблокировать
                $childBar->checkin('services.checkin');

                // Кнопка перемещения в корзину
                if ($this->state->get('filter.published') != IshopComponent::CONDITION_TRASHED) {
                    $childBar->trash('services.trash')->listCheck(true);
                }
            }
