...
/**
 * Класс контроллера списка зон доставки
 * @since 1.0.0
 */
class ZonesController extends AdminController
{
    ...
    public function __construct($config = [], ?MVCFactoryInterface $factory = null, $app = null, $input = null)
    {
        ...
        $this->registerTask('update', 'updateProductsDelivery');
    }

    /**
     * Обновление ближайший дат доставки
     * по зонам
     *
     * @throws \Exception
     * @since 1.0.0
     */
    public function updateProductsDelivery()
    {
        if (DeliveryHelper::update()) {
            $this->setMessage(Text::_('COM_ISHOP_EXPORT_ZONES_DELIVERY_SUCCESS'));
        } else {
            $this->setMessage(Text::_('COM_ISHOP_EXPORT_ZONES_DELIVERY_ERROR'), 'error');
        }

        $this->setRedirect(Route::_('index.php?option=com_ishop&view=zones', false));
    }
     ...
     * @since 1.0.0
     */
    public function getModel($name = 'Zone', $prefix = 'Administrator', $config = ['ignore_request' => true]): object
    {
